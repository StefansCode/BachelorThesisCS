\section{Entwicklung der Signalverarbeitungsbibliothek}
\label{chap:signalbibliothek}

Die DSV-Bibliothek wird als eine Klasse namens Signal implementiert.
Die Klasse ist eine Fluent API\cite{fluentAPI}. Das bedeutet, dass die
nicht abschließenden Funktionen eine Referenz auf das Signalobjekt zurückgeben. 
Dadurch können Methodenaufrufe verkettet werden, um komplexe Operationen auf 
das Signal anzuwenden.

Eine Funktion der Signalklasse stellt ein System dar, da sie ebenfalls
ein Signal in ein anderes Signal umwandelt.

Im Folgenden wird die Entwicklung der Signalklasse Schritt für Schritt
beschrieben. Bei jedem Schritt wird der angegebene Codeblock erweitert,
bis am Ende die Klasse vorliegt, wie sie zur Zeit der Arbeit vorliegt.

\subsection{Die Signalklasse}

Ein digitales Signal wird in seiner simpelsten Form als eine sich ständig
ändernde Variable dargestellt. Der Datentyp der Variable wird dem Nutzer 
überlassen, indem die Signalklasse als Template implementiert wird. 

\begin{lstlisting}[language=C++, 
                       frame=single, 
                       caption={Signalklasse Version 0.1},
                       captionpos=b]
template<typename T>
class Signal {
  T value;
}
\end{lstlisting}

Da oft auch die vorherigen Werte benötigt werden lohnt es sich diese ebenfalls 
zu speichern. Das ist zum Beispiel hilfreich, weil dem DAC ein Buffer übergeben 
wird.
Dafür bietet sich ein RingBuffer (circular buffer) \cite{ringbuffer} an. 
Ein Ringbuffer ist ein fest definierter
Speicherblock (zum Beispiel ein Array) mit fester Größe. Das ist vor allem
auf Mikrocontrollern sehr 
wichtig, da hier Speicher und Rechenkapazität begrenzt sind. Ein Ringbuffer hat außerdem
einen Index Wert welcher anzeigt, wo in diesen Speicherblock geschrieben 
werden soll. Der Index wird bei jedem schreiben erhöht und fängt wieder am Anfang
des Speicherblocks anzuschreiben, wenn das Ende erreicht ist.
Die Größe des Speichers soll auch den Nutzer überlassen werden.

\begin{lstlisting}[language=C++, 
                       frame=single, 
                       caption={Signalklasse Version 0.2},
                       captionpos=b]
template <typename T, unsigned int BUFFER_SIZE = 256>
class Signal {
private:
  T buffer[BUFFER_SIZE] = {0};
  unsigned int index = 0;
}
\end{lstlisting}

Wenn das Signal zu einem neuen Wert übergeht, muss der Index erhöht werden.
Das passiert in einer Inputfunktion, welche mit dem Prefix \glqq from-\grqq\ 
gekennzeichnet sind. Eine Inputfunktion gibt immer eine Referenz auf das 
Signalobjekt zurück und erhöht den Index.

Beim Schreiben des ersten Wertes wird der Index bereits erhöht. Der erste Wert
sollte jedoch an der ersten Stelle des Buffers gespeichert werden. Deshalb wird
der Index bei der Initialisierung auf BUFFER\_SIZE - 1 gesetzt.

\begin{lstlisting}[language=C++, 
                       frame=single, 
                       caption={Signalklasse Version 0.3},
                       captionpos=b]
template <typename T, unsigned int BUFFER_SIZE = 256>
class Signal {
private:
  T buffer[BUFFER_SIZE] = {0};
  unsigned int index = BUFFER_SIZE - 1;
public:
  Signal& fromValue(T value) {
    index = (index + 1) % BUFFER_SIZE;
    buffer[index] = value;
    return *this;
  }
}
\end{lstlisting}

Eine Funktion, die mit dem Prefix \glqq return-\grqq\ 
gekennzeichnet sind, ist eine Ausgangsfunktion. 
Solche Funktionen geben keine Signalreferenz zurück, sondern zum Beispiel den 
momentanen Wert.

\begin{lstlisting}[language=C++, 
                       frame=single, 
                       caption={Signalklasse Version 0.4},
                       captionpos=b]
template <typename T, unsigned int BUFFER_SIZE = 256>
class Signal {
private:
  T buffer[BUFFER_SIZE] = {0};
  unsigned int index = BUFFER_SIZE - 1;
public:
  ...

  T returnValue() {
    return buffer[index];
  }
}
\end{lstlisting}

Alle anderen Funktionen sind Funktionen, welche eine Signalreferenz zurückgeben
und im Gegensatz zu Inputfunktionen nicht den Index erhöhen. Das sind zum Zeitpunkt
der Arbeit die Funktionen
add, multiply oder applyFunction. Letzteres erlaubt es dem Nutzer selbstdefinierte 
Funktionen in die Verarbeitungskette einzufügen.

\begin{lstlisting}[language=C++, 
                       frame=single, 
                       caption={Signalklasse Version 0.5},
                       captionpos=b]
template <typename T, unsigned int BUFFER_SIZE = 256>
class Signal {
private:
  T buffer[BUFFER_SIZE] = {0};
  unsigned int index = BUFFER_SIZE - 1;
public:
  ...

  Signal& add(Signal &other) {
    this->buffer[index] += other.buffer[other.index];
    return *this;
  }

  Signal& multiply(T value) {
    this->buffer[index] = T(this->buffer[index] * value);
    return *this;
  }

  Signal& applyFunction(T (*func)(T data, void *param), 
                       void *param) {

    buffer[index] = func(buffer[index], param);
    return *this;
  }
}
\end{lstlisting}

\subsection{Eine Anmerkung zur Weiterentwicklung}

Soll ein System implementiert werden, das auf vorherige Werte zugreift, 
muss folgendes beachtet werden.

Sei $F$ ein System, das den aktuellen Wert auf den Mittelwert der letzten 
5 Werte aus dem Buffer setzt.
Wenn es noch keine 5 Werte gibt, so wird über die Anzahl der Werte gemittelt.

Nehme also an $F$ ist in Signal implementiert und betrachte folgende Anwendung 
von Signal, in der zuerst $F$ angewendet wird und dann mit 10 multipliziert wird.

Besteht nun das eingehende Signal nur aus dem sich wiederholenden Wertem 1,
passiert der in Abbildung \ref{fig:signal-buffer-problem} dargestellte Fehler.

\begin{figure}[h]
    \centering
    \begin{tikzpicture}[
        >=stealth,
        node distance=0.7cm,
        every node/.style={font=\small},
        box/.style={
            draw,
            rounded corners,
            minimum height=0.8cm,
            minimum width=1.2cm,
            align=center
        }
    ]

        % 1. Iteration
        \node (in1) {$1$};
        \node[right=of in1] (buf1) {$[1,0,0,0,0]$};
        \node[box, right=of buf1] (filter1) {$F$};
        \node[right=of filter1] (buf2) {$[1,0,0,0,0]$};
        \node[box, right=of buf2] (mul1) {$\times 10$};
        \node[right=of mul1] (out1) {$[10,0,0,0,0]$};

        \draw[->] (in1) -- (buf1);
        \draw[->] (buf1) -- (filter1);
        \draw[->] (filter1) -- (buf2);
        \draw[->] (buf2) -- (mul1);
        \draw[->] (mul1) -- (out1);

        % 2. Iteration
        \node[below=1.5cm of in1] (in2) {$1$};
        \node[right=of in2] (buf3) {$[10,1,0,0,0]$};
        \node[box, right=of buf3] (filter2) {$F$};
        \node[right=of filter2] (buf4) {$[10,5.5,0,0,0]$};
        \node[box, right=of buf4] (mul2) {$\times 10$};
        \node[right=of mul2] (out2) {$[10,55,0,0,0]$};

        \draw[->] (in2) -- (buf3);
        \draw[->] (buf3) -- (filter2);
        \draw[->] (filter2) -- (buf4);
        \draw[->] (buf4) -- (mul2);
        \draw[->] (mul2) -- (out2);

        \node[
            red,
            align=center,
            below=0.5cm of filter2
        ] (problem)
        {Hier greift $F$ auf den mit \\
        $10$ multiplizierten Wert zu.};

        \draw[->, red, thick]
            (problem.north) -- (filter2.south);

        % Beschriftungen
        \node[left=0.5cm of in1] {\textbf{1. Iteration}};
        \node[left=0.5cm of in2] {\textbf{2. Iteration}};

    \end{tikzpicture}
    \caption{Problem bei der Verwendung eines gemeinsamen Buffers.}
    \label{fig:signal-buffer-problem}
\end{figure}

Bei der zweiten Iteration müsste $F$ eigentlich über zwei Einsen mitteln, aber
die vorherige Eins wurde schon in der ersten Iteration mit 10 multipliziert.

Eine Lösung wäre es $F$ und jeder anderen Funktion, die einen Buffer braucht,
beim Aufruf einen zu übergeben. 

\subsection{Anwendungsbeispiel der Signalklasse}
% wohin soll das Example?
Im Anhang \ref{example:DSV} sieht man eine Beispielanwendung in der ArduinoIDE. 
Hier werden ein
lichtabhängiger Widerstand und ein Potentiometer eingelesen. Der Potentiometerwert
wird dazu verwendet, um den aktuell stabilen Lichtwert zu kompensieren. Dann wird
eine LED angeschaltet, wenn der kompensierte Lichtwert unter 0 sinkt.

In diesem Beispiel ist es ersichtlich, dass die Signalklasse 
leicht durch Funktionen wie einen Mittelwert oder eine Printfunktion erweitert
werden kann.

\subsection{Anmerkungen}
Ein großer Unterschied zwischen der Signalklasse und Java Streams ist, dass 
die Inputfunktionen der Signalklasse keine statischen Memberfunktionen sind.
Das liegt daran, dass ein Signal im Normalfall in einer Endlosschleife 
aktualisiert wird und nicht wie ein Stream einmal erschaffen und direkt 
beendet wird. 

Die Abtastrate hängt davon ab, wie oft pro Sekunde eine Inputfunktion
augerufen wird.

